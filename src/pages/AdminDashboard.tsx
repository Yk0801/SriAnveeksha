      ...
      {student && (
        <>
          ...
          <form onSubmit={addRemark} className="bg-slate-50 border border-slate-200 rounded-xl p-4 mb-5 grid sm:grid-cols-2 gap-3">
            ...
            <div className="sm:col-span-2">
              <button type="submit" disabled={saving} className="bg-[#d4af37] text-white font-bold text-xs px-5 py-2.5 rounded-lg disabled:opacity-60">{saving ? "Adding…" : "Add Remark"}</button>
            </div>
          </form>
          <table className="portal-table">
...
const MarksAdminPanel = () => {
  ...
  return (
    <div>
      ...
      <div className="flex gap-3 mb-4">
        <input type="text" placeholder="Search student name or admission no." value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === "Enter" && searchStudent()} className="flex-1 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#d4af37]/30" style={{ fontFamily: "Inter,sans-serif" }} />
        <button onClick={searchStudent} className="bg-[#d4af37] text-white font-bold text-xs px-4 py-2 rounded-lg">{loading ? "…" : "Search"}</button>
      </div>
      {student && (
        <>
          ...
          <form onSubmit={addMark} className="bg-slate-50 border border-slate-200 rounded-xl p-4 mb-5 grid sm:grid-cols-2 md:grid-cols-5 gap-3 items-end">
            ...
            <div className="sm:col-span-2 md:col-span-5 pt-2">
              <button type="submit" disabled={saving} className="bg-[#d4af37] text-white font-bold text-xs px-5 py-2.5 rounded-lg disabled:opacity-60">{saving ? "Recording…" : "Save Marks"}</button>
            </div>
          </form>
          <table className="portal-table">
        ...
        {[
          {
            ...
            content: (
              <div className="grid sm:grid-cols-2 gap-3">
                ...
                <div className="sm:col-span-2"><button onClick={() => toast.success("Settings saved!")} className="bg-[#d4af37] text-white font-bold text-xs px-5 py-2 rounded-lg">Save</button></div>
              </div>
            )
          },
          {
            ...
            content: (
              <div className="flex gap-4 flex-wrap">
                ...
                <div className="w-full"><button onClick={() => toast.success("Year saved!")} className="bg-[#d4af37] text-white font-bold text-xs px-5 py-2 rounded-lg">Save</button></div>
              </div>
            )
          },
        ].map(sec => (
