    ...
    <section id="books" className="py-20 bg-slate-50">
      <div className="max-w-6xl mx-auto px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
          <p className="text-[#F97316] font-semibold text-sm uppercase tracking-widest mb-3" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Stationery & Books</p>
          ...
          {classData.map((c, i) => (
            <motion.div key={c.cls} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              ...
              <button onClick={() => toggle(c.cls)}
                ...
                <span className={`font-bold text-lg transition-colors ${open === c.cls ? 'text-[#F97316]' : 'text-slate-800 group-hover:text-[#F97316]'}`}>{c.cls} — Complete List</span>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${open === c.cls ? 'bg-orange-50' : 'bg-slate-50'}`}>
                  <ChevronDown className={`w-5 h-5 transition-transform duration-300 ${open === c.cls ? "rotate-180 text-[#F97316]" : "text-slate-400"}`} />
                </div>
              </button>
              <AnimatePresence>
                {open === c.cls && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                    ...
                    <div className="px-6 pb-6 grid md:grid-cols-3 gap-6 border-t border-slate-100 pt-5">
                      {[
                        ...
                        <div key={cat.label} className="bg-orange-50/30 rounded-xl p-4 border border-orange-50">
                          ...
                          <ul className="space-y-2">
                            {cat.items.map((item) => (
                              <li key={item} className="text-slate-600 text-xs flex items-start gap-2 leading-relaxed" style={{ fontFamily: "Inter, sans-serif" }}>
                                <span className="text-[#F97316] flex-shrink-0 mt-0.5">•</span>{item}
                              </li>
                            ))}
                          </ul>
                        </div>
                    ...
                    <div className="px-6 pb-6 flex justify-center sm:justify-start">
                      <button className="text-[#F97316] font-semibold text-sm px-6 py-2.5 rounded-lg border border-[#F97316]/30 hover:bg-[#F97316] hover:text-white transition-colors">
