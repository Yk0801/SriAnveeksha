  ...
  <section id="bus" className="py-20 bg-white">
    <div className="max-w-6xl mx-auto px-6">
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
        <p className="text-[#d4af37] font-semibold text-sm uppercase tracking-widest mb-3" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Transport</p>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="overflow-x-auto rounded-2xl border border-slate-100 shadow-sm mb-10">
        <table className="w-full text-sm">
          ...
          <tbody>
            {routes.map((r, i) => (
              <tr key={r.routeNo} className={`${i % 2 === 0 ? "bg-white" : "bg-slate-50"} hover:bg-gold-50 transition-colors`}>
                <td className="px-4 py-3 font-bold text-[#d4af37]" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{r.routeNo}</td>
                ...
                <td className="px-4 py-3 text-[#0c2340] font-medium" style={{ fontFamily: "Inter, sans-serif" }}>{r.mobile}</td>
              </tr>
            ))}
          </tbody>
        </table>
