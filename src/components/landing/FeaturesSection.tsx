    ...
    <section id="features" className="py-20 bg-white">
      <div className="max-w-6xl mx-auto px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
          <p className="text-[#d4af37] font-semibold text-sm uppercase tracking-widest mb-3" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Why Choose Us</p>
          ...
          <div className="absolute bottom-6 right-6 flex gap-2 z-20">
            {features.map((_, i) => (
              <button 
                ...
                className={`w-2.5 h-2.5 rounded-full transition-all ${i === current ? "bg-[#d4af37] w-8" : "bg-white/50 hover:bg-white"}`}
              />
            ))}
          </div>
        </div>
