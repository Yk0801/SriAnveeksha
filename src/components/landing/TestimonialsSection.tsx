...
const TestimonialsSection = () => {
  ...
  return (
    <section id="testimonials" className="py-20 bg-gradient-to-br from-[#F97316]/5 to-[#4F46E5]/5">
      <div className="max-w-4xl mx-auto px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
          <p className="text-[#F97316] font-semibold text-sm uppercase tracking-widest mb-3" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>What Parents Say</p>
            ...
            >
              <div className="flex justify-center gap-1 mb-4">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} className={`w-5 h-5 ${i < t.stars ? "text-[#F97316] fill-[#F97316]" : "text-slate-200"}`} />
                ))}
              </div>
        ...
        <div className="flex justify-center gap-2 mt-4">
          {testimonials.map((_, i) => (
            <button key={i} onClick={() => setIdx(i)}
              className={`w-2 h-2 rounded-full transition-all ${i === idx ? "bg-[#F97316] w-6" : "bg-slate-300"}`} />
          ))}
        </div>
          ...
          {reviewSubmitted ? (
            <div className="text-center py-6">
              <div className="w-12 h-12 bg-[#F97316]/10 rounded-full flex items-center justify-center mx-auto mb-3">
                <Check className="w-6 h-6 text-[#F97316]" />
              </div>
              <p className="font-semibold text-slate-800 mb-1" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Thank you for your review!</p>
              <button onClick={() => setReviewSubmitted(false)} className="text-[#F97316] text-sm hover:underline mt-1">Submit another</button>
            </div>
          ) : (
            <form onSubmit={(e) => { e.preventDefault(); setReviewSubmitted(true); }} className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" required value={review.name} onChange={e => setReview({ ...review, name: e.target.value })}
                  className="w-full px-3 py-2.5 rounded-lg border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#F97316]/30 focus:border-[#F97316]"
              ...
              <div className="sm:col-span-2">
                ...
                <textarea required rows={3} value={review.quote} onChange={e => setReview({ ...review, quote: e.target.value })}
                  className="w-full px-3 py-2.5 rounded-lg border border-slate-200 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-[#F97316]/30 focus:border-[#F97316]"
              ...
              <div className="sm:col-span-2 flex items-center gap-3">
                ...
                {[1, 2, 3, 4, 5].map((s) => (
                  <button key={s} type="button" onClick={() => setReview({ ...review, stars: s })}>
                    <Star className={`w-6 h-6 transition-colors ${s <= review.stars ? "text-[#F97316] fill-[#F97316]" : "text-slate-300"}`} />
                  </button>
                ))}
              </div>
              <div className="sm:col-span-2">
