    ...
    <section id="cocurricular" className="py-20 bg-slate-50">
      <div className="max-w-6xl mx-auto px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
          <p className="text-[#d4af37] font-semibold text-sm uppercase tracking-widest mb-3" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Beyond the Classroom</p>
          ...
          <div className="absolute bottom-8 right-8 flex gap-2.5 z-20">
            {carouselActivities.map((_, i) => (
              <button 
                ...
                className={`h-2.5 rounded-full transition-all duration-300 ${i === current ? "bg-[#d4af37] w-10 relative overflow-hidden" : "w-2.5 bg-white/50 hover:bg-white"}`}
              >
                {i === current && (
                  <motion.div 
